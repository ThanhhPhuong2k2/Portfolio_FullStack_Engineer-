  ...
  return (
    <div className="relative bg-[#020617] text-slate-100 selection:bg-sky-500/30 overflow-x-hidden min-h-screen cursor-none font-sans">
      ...
      <motion.div 
        ...
        style={{
          background: useTransform(
            ...
            ([x, y, color]) => `radial-gradient(1100px circle at ${x}px ${y}px, ${color}, transparent 80%)`
          )
        }}
      />
      ...
      <Navbar 
        ...
        onCloseDetail={(selectedProject || viewAllProjects) ? () => { setSelectedProject(null); setViewAllProjects(false); } : undefined}
      />
      ...
      <AnimatePresence mode="wait">
        {!selectedProject ? (
          viewAllProjects ? (
            ...
            {/* Hero Section */}
            <section id="home" className="relative z-10 pt-20 md:pt-40 pb-20 px-6 min-h-screen flex items-center justify-center perspective-[2000px]">
              <TechBackground />
                    ...
                    >
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-[10px] font-mono font-black text-sky-400/80 tracking-widest uppercase">CORE_IDENTITY</span>
                        </div>
                        <div className="flex items-baseline gap-2">
                          <span className="text-2xl md:text-4xl font-black text-white tracking-tighter">02.01</span>
                          <span className="text-sm md:text-xl font-bold text-slate-500 italic">/ 2002</span>
                        </div>
                        <div className="h-px w-full bg-white/10 my-1" />
                        <span className="text-[9px] font-mono text-slate-400 uppercase tracking-widest font-bold">DA NANG, VN</span>
                      </div>
                    </motion.div>
                  ...
                  <div className="flex-1 text-center lg:text-left relative z-10">
                    ...
                    >
                      ...
                      <span className="text-sky-400 text-[10px] md:text-[13px] font-black uppercase tracking-[0.4em]">
                        {lang === 'en' ? 'Fullstack Architect' : 'Kiến trúc sư Fullstack'}
                      </span>
                    </motion.div>
                    ...
                    >
                      <div className="text-gradient flex flex-wrap items-center justify-center lg:justify-start drop-shadow-[0_0_30px_rgba(56,189,248,0.2)]">
                        {splitName("HO VAN THANH PHUONG").map((l, i) => (
                          <motion.span key={`fullname-${i}`} variants={letterVars} className="inline-block whitespace-pre">{l}</motion.span>
                        ))}
                      </div>
                    </motion.h1>
                    ...
                    >
                      <p className="text-lg md:text-2xl text-slate-400 font-light leading-relaxed italic border-l-4 border-sky-500/30 pl-8">
                        {lang === 'en' 
                          ? <>Striving to become an <span className="text-white font-bold">exceptional Fullstack Developer</span>, aiming to build high-impact products and effectively solve complex systemic challenges.</>
                          : <>Hướng tới trở thành một <span className="text-white font-bold">Lập trình viên Fullstack xuất sắc</span>, với mục tiêu xây dựng các sản phẩm mang giá trị thực tiễn và giải quyết hiệu quả những bài toán hệ thống phức tạp.</>}
                      </p>
                    </motion.div>
                  </div>
                </div>
            ...
            {/* Projects Section */}
            <section id="experience" className="relative z-10 py-24 px-6 bg-slate-950/20">
              <div className="container mx-auto">
                <div className="mb-16 text-center">
                  ...
                  >
                    ...
                    <span className="text-gradient">{lang === 'en' ? 'PROJECTS' : 'TIÊU BIỂU'}</span>
                  </motion.h2>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
                  {PROJECTS.filter(p => p.title !== 'VIDE').slice(0, 4).map((project, idx) => (
                    <ProjectCard 
                      key={project.title} 
                      project={project} 
                      index={idx} 
                ...
                <div className="mt-16 text-center">
                  <motion.button
                    whileHover={{ scale: 1.05, boxShadow: '0 0 30px rgba(56,189,248,0.35)' }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => {
                      setViewAllProjects(true);
                      window.scrollTo({ top: 0, behavior: 'smooth' });
                    }}
                    className="relative px-12 py-5 rounded-2xl bg-slate-900 border border-white/10 text-white font-bold transition-all shadow-2xl inline-flex items-center gap-4 group uppercase tracking-[0.18em] text-[10px] md:text-xs cursor-pointer overflow-hidden hover:border-sky-500/30"
                  >
                    <span className="absolute inset-0 bg-gradient-to-r from-sky-500/10 via-purple-500/10 to-pink-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                    <span className="relative z-10 text-slate-200 group-hover:text-white">{lang === 'en' ? 'View All Projects' : 'Xem Tất Cả Dự Án'}</span>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-sky-400 transform group-hover:translate-x-2 transition-transform duration-300 relative z-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <section id="journey" className="relative z-10 py-24 bg-slate-950/40">
              <div className="container mx-auto px-6 mb-16 text-center">
                <motion.h2 className="text-3xl md:text-7xl font-black tracking-tighter text-white uppercase leading-none">
                  {lang === 'en' ? 'MY' : 'HÀNH'}{' '}
                  <span className="text-gradient">{lang === 'en' ? 'JOURNEY' : 'TRÌNH'}</span>
                </motion.h2>
              </div>
              ...
              <div className="container mx-auto">
                <div className="mb-16 text-center">
                  ...
                  >
                    ...
                    <span className="text-gradient">{lang === 'en' ? 'EXPERIENCE' : 'LÀM VIỆC'}</span>
                  </motion.h2>
                </div>
      ...
      <footer className="relative z-10 pt-16 pb-24 md:pb-12 text-center text-slate-600 text-[10px] font-mono border-t border-white/5 tracking-[0.6em] uppercase px-6">
        &copy; {new Date().getFullYear()} HO VAN THANH PHUONG // V2.5 // CORE_ACTIVE
      </footer>
    </div>
  );
};
